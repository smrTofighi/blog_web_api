...
function generateReplyPrompt(comment) {
  ...
  return `You are replying to a blog comment by ${authorName}.

The comment says:
"${content}"

Write a friendly, thoughtful, concise (max 2-3 sentences), and relevant reply to this comment.
The goal of the reply is to: thank the commenter, acknowledge a positive point they made, or add a brief, complementary detail related to their comment.
Avoid generic responses or bringing up external topics. Focus strictly on the content of their comment.`;
}
...
- Under that heading, list 3👍 key takeaways or skills the reader will learn in **bullet points** using markdown (\`- \`).

Return the result in **valid JSON** with the following structure:

{
    "title": "Short SEO-friendly title",
    "summary": "300_word summary with a markdown section for What You'll Learn"
}
